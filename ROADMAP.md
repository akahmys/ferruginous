...
### Phase 18: Multibyte Text Rendering Precision (マルチバイトテキスト描画の精密化) [要再検証]

> [!WARNING]
> 本フェーズは複数セッションで「完了」と報告されたが、自動テストによる実証がなく、
> リグレッションの繰り返しが確認されている。完了ゲート（テスト PASS + ユーザー承認）を
> 通過するまで「完了」とみなさない。

- **M59-S: FontMatrix 標準化とコンポジットフォント同期 (SDK)** [要再検証]
  - ISO 32000-2 準拠のデフォルト FontMatrix (0.001) の統一と、Type 0 フォントにおける子要素 CIDFont からの行列継承の実装。
- **M60-S: グリフ幅の厳密同期 (SDK)** [要再検証]
  - PDF の `W` (Widths) 配列とフォントファイルのグリフメトリクスを強制同期させ、日本語文字の重なり（バンチング）を物理的に解消。
- **M61-A: レンダラー座標変換の完全同期 (UI)** [要再検証]
