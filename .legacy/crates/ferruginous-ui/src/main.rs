        ...
        if let Some(render_state) = frame.wgpu_render_state() {
            ...
            if self.vello_texture_id.is_none() {
                ...
                if let Some(target_view) = &self.vello_texture_view {
                    let _width = ((self.current_page_size.x * SUPER_SAMPLE_FACTOR).max(1.0) as u32).min(4096);
                    let _height = ((self.current_page_size.y * SUPER_SAMPLE_FACTOR).max(1.0) as u32).min(4096);
