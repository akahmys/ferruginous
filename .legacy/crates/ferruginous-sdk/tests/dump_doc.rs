...
#[test]
fn test_dump_c2_1() {
    let pdf_path = "../../samples/legacy/small-ja.pdf";
    ...
    for i in 1..2000 {
        ...
        if let Ok(Object::Dictionary(dict)) = doc.resolver().resolve(&reference) {
            if let Some(Object::Name(n)) = dict.get(b"Type".as_ref()) {
                if **n == *b"Font" {
                    ...
                    if subtype.as_deref() == Some("Type0") {
                        ...
                        if let Some(df) = dict.get(b"DescendantFonts".as_ref()) {
