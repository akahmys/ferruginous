...
impl<'a> Catalog<'a> {
    ...
    #[must_use] pub fn metadata(&self) -> Option<Metadata> {
        ...
        self.dictionary.get(b"Metadata".as_ref()).and_then(|obj| {
            ...
            match obj {
                Object::Stream(dict, content) => Some(Metadata::new(std::sync::Arc::clone(dict), content.clone())),
                Object::Reference(r) => {
    ...
    /// Retrieves the document OutputIntents (Clause 14.11).
    pub fn output_intents(&self) -> PdfResult<Vec<crate::color::output_intent::OutputIntent>> {
        let obj = match self.dictionary.get(b"OutputIntents".as_ref()) {
            Some(o) => o,
            None => return Ok(Vec::new()),
        };

        let arr = match self.resolver.resolve_if_ref(obj)? {
            Object::Array(a) => a,
        ...
        for item in arr.iter() {
            let dict = self.resolver.resolve_if_ref(&item)?.as_dict_arc()
                .ok_or_else(|| PdfError::InvalidType { expected: "Dictionary".into(), found: "OutputIntent item".into() })?;
            out.push(crate::color::output_intent::OutputIntent::from_dict(&dict, self.resolver)?);
        }
        Ok(out)
    }
}
