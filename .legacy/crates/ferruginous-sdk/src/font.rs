...
pub struct FontDescriptor {
    ...
    pub font_data: Option<bytes::Bytes>,
}
...
impl Font {
    ...
    pub fn from_dict(dict: &BTreeMap<Vec<u8>, Object>, resolver: &dyn Resolver) -> PdfResult<Self> {
        ...
        let mut is_multi_byte = subtype == b"Type0" || subtype == b"CIDFontType0" || subtype == b"CIDFontType2";

        // HEURISTIC: Many Japanese PDFs subset fonts in a way that looks like Type1 
        // but they are actually encoded multi-byte or CID-keyed.
        let is_jp = base_font.len() >= 4 && base_font.windows(4).any(|w| w == b"Hira" || w == b"KozM" || w == b"Ryumin" || w == b"Goth" || w == b"MS-P");
        if is_jp && !is_multi_byte {
            eprintln!("[DEBUG][FONT] Heuristic matched: treating {:?} as multi-byte JP font", String::from_utf8_lossy(&base_font));
            is_multi_byte = true;
        }

        eprintln!("[TRACE][FONT] Loading font: BaseFont={:?}, Subtype={:?}, WMode={}", 
            String::from_utf8_lossy(&base_font), String::from_utf8_lossy(&subtype), wmode);

        if let Some(encoding) = dict.get(b"Encoding".as_ref()) {
            eprintln!("[TRACE][FONT] Encoding found: {:?}", encoding);
            encoding_cmap = Self::resolve_cmap(encoding, resolver).unwrap_or(None);
        }
    ...
    #[must_use] pub fn cid_width(&self, cid: u32) -> f64 {
        // Relaxed assertion: If we have CID mappings, we should be able to look up widths 
        // regardless of the nominal subtype (e.g. Type1 with CID mapping).
    ...
    /// Resolves a character code to a Glyph ID (GID).
    pub fn resolve_gid(&self, code: &[u8]) -> Option<u32> {
        // 1. Priority: CID-to-GID Map
        if let Some(ref map) = self.cid_to_gid_map {
            if let Some(MappingResult::Cid(cid)) = self.encoding_cmap.as_ref().and_then(|m| m.lookup(code)) {
                let gid = map.get(cid as usize).copied().map(|id| id as u32);
                if gid.is_some() { return gid; }
            }
        }

        // 2. Fallback: TrueType cmap lookup / CMap CID
        if let Some(MappingResult::Cid(cid)) = self.encoding_cmap.as_ref().and_then(|m| m.lookup(code)) {
             return Some(cid as u32);
        }

        // 3. Fallback: UNICODE mapping lookup (essential for system fallback fonts)
        let unicode = self.to_unicode_string(code);
        if !unicode.is_empty() {
             // We return a 'sentinel' that indicates we should use Unicode lookup in the font file
             // For now, let's keep it simple and see if 1-byte fallback is hit.
        }

        // 4. Fallback: Byte value as GID
        if code.len() == 1 {
            return Some(code[0] as u32);
        }

        eprintln!("[DEBUG][FONT] GID resolution failed for code: {:?}", code);
        None
    }
    ...
    pub fn get_glyph_path(&self, code: &[u8]) -> PdfResult<Option<(BezPath, f64)>> {
        // 1. Resolve GID/CID
        let gid = self.resolve_gid(code);
        if gid.is_none() {
             eprintln!("[DEBUG][FONT] GID resolution returned None for code: {:?}", code);
             return Ok(None);
        }
        ...
        if let Some(ref desc) = self.descriptor {
            if let Some(ref data) = desc.font_data {
                embedded_data_ref = Some(data.as_ref());
            }
        }
                        ...
                        let data = SYSTEM_FONT_CACHE.get_or_init(|| {
                            ...
                            for f in fonts {
                                if let Ok(d) = std::fs::read(f) {
                                    eprintln!("[TRACE][SDK] Loaded system fallback font: {f}");
                                    return d;
                                }
                            }
                            eprintln!("[ERROR][SDK] No Japanese system fallback font found!");
                            Vec::new()
                        });
                        if !data.is_empty() {
                            data_ref = Some(data);
        ...
        if let Ok(face) = ttf_parser::Face::parse(font_data, 0) {
            ...
            let glyph_id_val = if let Some(g) = gid {
                g
            } else {
                ...
                let gid_obj = if _system_font_fallback || (!self.subtype.starts_with(b"CIDFont") && self.subtype != b"Type0") {
                    let unicode_str = self.to_unicode_string(code);
                    let gid = if let Some(c) = unicode_str.chars().next() {
                        face.glyph_index(c).unwrap_or(ttf_parser::GlyphId(0))
                    } else {
                        ttf_parser::GlyphId(0)
                    };

                    if gid.0 == 0 && !self.is_multi_byte() && code.len() == 1 {
                        // Fallback: byte value might be the GID in subsetted fonts
                        ttf_parser::GlyphId(code[0] as u16)
                    } else {
                        gid
                    }
                    ...
                    if let Some(MappingResult::Cid(cid)) = cmap.lookup(code) {
                    ...
                    } else if _system_font_fallback {
                        // For system fallback, we MUST use Unicode
                        let unicode_str = self.to_unicode_string(code);
                        ...
                        // HEURISTIC: For some TrueType subset fonts, the CID is just the byte
                        ttf_parser::GlyphId(code[0] as u16)
            ...
            let glyph_id = ttf_parser::GlyphId(glyph_id_val as u16);
            
            if glyph_id.0 == 0 && !_system_font_fallback { 
                eprintln!("[TRACE][SDK] GID 0 for code: {code:?}, font={}", base_font_str);
                return Ok(None); 
            }

            let mut builder = BezPathBuilder::new();
            let result = face.outline_glyph(glyph_id, &mut builder);
            if result.is_some() {
                ...
                let native_advance = face.glyph_hor_advance(glyph_id).map_or(face.units_per_em() as f64, |w| w as f64) * scale;
            ...
            if let Some(Object::Reference(r)) = dict.get(key) {
                if let Ok(Object::Stream(s_dict, data)) = resolver.resolve(r) {
                    if let Ok(decoded) = crate::filter::decode_stream(&s_dict, &data) {
                        font_data = Some(bytes::Bytes::from(decoded));
                        break;
                    }
                }
