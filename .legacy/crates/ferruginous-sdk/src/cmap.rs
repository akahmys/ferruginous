...
pub struct CMap {
    ...
    pub bf_chars: BTreeMap<Vec<u8>, bytes::Bytes>,
...
impl CMap {
    ...
    pub fn parse(input: &[u8]) -> PdfResult<Self> {
        ...
        while !current_input.is_empty() {
            ...
            // 1. Check for WMode and other metadata via simple scans if lexing fails or as a complement
            if current_input.starts_with(b"/WMode") || current_input.starts_with(b"WMode") {
                let start_idx = if current_input.starts_with(b"/") { 6 } else { 5 };
                let after_key = &current_input[start_idx..];
                if let Ok((val_next, Object::Integer(v))) = parse_object(after_key) {
                    eprintln!("[TRACE][CMAP] Robustly found WMode: {}", v);
                    if v == 1 { cmap.is_vertical = true; }
            ...
            if current_input.starts_with(b"beginbfchar") {
                ...
                if after_spaces.starts_with(b"usecmap") {
                    ...
                    if key == "WMode" || key == "/WMode" {
                        if let Ok((val_next, Object::Integer(v))) = parse_object(after_spaces) {
                            eprintln!("[TRACE][CMAP] Found WMode with value: {}", v);
                            if v == 1 { cmap.is_vertical = true; }
                            current_input = val_next;
                            continue;
                        }
                    } else if key == "CMapName" || key == "/CMapName" {
                        if let Ok((val_next, Object::Name(name))) = parse_object(after_spaces) {
                            cmap.name = String::from_utf8_lossy(&name).into_owned();
                            if cmap.name.ends_with("-V") || cmap.name.contains("Vertical") {
                                cmap.is_vertical = true;
                            }
                            current_input = val_next;
                            continue;
    ...
    fn parse_mapping_block<'a>(input: &'a [u8], end_tag: &str) -> PdfResult<(&'a [u8], Vec<(Vec<u8>, bytes::Bytes)>)> {
    ...
    #[must_use] pub fn lookup(&self, code: &[u8]) -> Option<MappingResult> {
        ...
        for range in &self.bf_ranges {
            if code >= range.start.as_slice() && code <= range.end.as_slice() {
                let offset = Self::compute_offset(code, &range.start);
                let mut res = range.dst.to_vec();
                if !res.is_empty() {
                    let last = res.len() - 1;
                    res[last] = res[last].wrapping_add(offset as u8);
                }
                return Some(MappingResult::Unicode(bytes::Bytes::from(res)));
            }
        }
