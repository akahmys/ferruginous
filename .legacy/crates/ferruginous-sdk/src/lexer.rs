...
fn parse_stream(input: &[u8]) -> IResult<&[u8], Object> {
    ...
    if let Some(Object::Integer(len)) = dict.get(b"Length".as_slice()) {
        ...
        if input.len() >= len_usize {
            ...
            return Ok((remaining, Object::new_stream_bytes(std::sync::Arc::clone(dict), bytes::Bytes::from(bytes))));
        }
    }
    ...
    Ok((input, Object::new_stream_bytes(std::sync::Arc::clone(dict), bytes::Bytes::from(bytes))))
}
