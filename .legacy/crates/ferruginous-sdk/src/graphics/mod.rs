...
impl Color {
    ...
    pub fn to_rgb(&self, space: &crate::colorspace::ColorSpace, bpc: bool, intent: lcms2::Intent) -> [f32; 3] {
        match self {
            Self::Gray(g) => [*g, *g, *g],
            Self::RGB(r, g, b) => [*r, *g, *b],
            #[allow(clippy::many_single_char_names)]
            Self::CMYK(c, m, y, k) => {
                // DeviceCMYK -> RGB conversion (can be enhanced with OutputIntent later)
                let r = (1.0 - c) * (1.0 - k);
                let g = (1.0 - m) * (1.0 - k);
                let b = (1.0 - y) * (1.0 - k);
                [r, g, b]
            }
            Self::ICC(components) | Self::Special(components) => space.to_rgb_ext(components, bpc, intent),
            Self::Lab(l, a, b) => space.to_rgb_ext(&[*l, *a, *b], bpc, intent),
...
impl TilingPattern {
    ...
    pub fn from_stream(dict: &BTreeMap<Vec<u8>, Object>, data: bytes::Bytes) -> Self {
