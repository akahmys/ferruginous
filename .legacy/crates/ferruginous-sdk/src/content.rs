...
pub struct Processor<'a> {
    ...
    /// Last detected text position for vertical flow inference.
    pub last_text_pos: Option<kurbo::Point>,
    /// Whether we have detected a vertical writing flow heuristic.
    pub inferred_vertical_flow: bool,
    /// The resolver for looking up indirect objects.
    pub resolver: &'a dyn crate::core::types::Resolver,
}

impl<'a> Processor<'a> {
    /// Creates a new Processor instance for executing content streams.
    pub fn new(resources: Option<Resources<'a>>, mediabox: Option<[f64; 4]>, oc_context: Option<crate::ocg::OCContext>, resolver: &'a dyn crate::core::types::Resolver) -> Self {
    ...
    fn current_visibility(&self) -> bool {
        let vis = *self.visibility_stack.last().unwrap_or(&true);
        if !vis {
            // eprintln!("[DEBUG][VISIBILITY] Current visibility is FALSE");
        }
        vis
    }
    ...
    pub fn execute_operation(&mut self, op: &Operation) -> PdfResult<Option<Vec<ContentNode>>> {
        ...
        let is_visible = self.current_visibility();
        if op_name == "Tj" || op_name == "TJ" {
             eprintln!("[DEBUG][CONTENT] Executing text op: {}, visible: {}", op_name, is_visible);
        }
    ...
    fn handle_tm(&mut self, op: &Operation) -> PdfResult<()> {
        let v = self.extract_f64_operands(&op.operands, 6);
        if v.len() == 6 {
            let new_pos = kurbo::Point::new(v[4], v[5]);
            
            // AUTO-DETECT VERTICAL FLOW: 
            // If X is constant but Y is decreasing significantly, it's vertical.
            if let Some(last) = self.last_text_pos {
                let dx = (new_pos.x - last.x).abs();
                let dy = new_pos.y - last.y; // Standard PDF: Y decreases downwards
                
                if dx < 0.01 && dy < -1.0 {
                    if !self.inferred_vertical_flow {
                        self.inferred_vertical_flow = true;
                    }
                } else if dx > 5.0 && dy.abs() < 1.0 {
                    // Reset if we see significant horizontal movement with no vertical movement
                    if self.inferred_vertical_flow {
                         self.inferred_vertical_flow = false;
                    }
                }
            }
            self.last_text_pos = Some(new_pos);
        ...
        if let Some(Object::String(bytes)) = op.operands.first() {
            ...
            if self.current_visibility() {
                let (glyphs_out, font_id, font_size, color_out, blend_mode, alpha) = {
                    let gs = self.gs_stack.current()?;
                    let intent = crate::colorspace::ColorSpace::map_intent(&gs.rendering_intent);
                    let resolved_color = gs.fill_color.to_rgb(&gs.fill_color_space, gs.black_point_compensation, intent);
                    (
                        glyphs.clone(),
    ...
    fn calculate_glyph_data(&self, char_code: &[u8], tm: Affine) -> (kurbo::Rect, Option<Arc<BezPath>>) {
        ...
        // 3. Matrix Composition (Trm = (Tfs * Th * Tmode) * Tm * CTM)
        // Here tm already accounts for CTM in our processor's state if applied correctly,
        // but typically tm is the text matrix which is then combined with graphics state.
        let text_extra = Affine::new([fs * th, 0.0, 0.0, fs, 0.0, rise]);
        
        // Path offset and glyph scale are in glyph space (0-1000 usually)
        // Order: Glyph -> GlyphScale -> PathOffset -> FontMatrix -> TextExtra -> Tm
        let final_transform = tm * text_extra * Affine::new(font.font_matrix) * path_offset * glyph_scale;
    ...
    fn compute_path_offset(&self, font: &crate::font::Font, char_code: &[u8]) -> Affine {
        let is_vertical = self.text_state.wmode == 1 || self.inferred_vertical_flow;
        if is_vertical {
            ...
            if font.char_should_rotate_vertical(char_code) {
                // Rotate 90 degrees around character center
        ...
        if let Some(Object::Name(name)) = op.operands.first() {
            if let Some(ref res) = self.resources {
                if let Some(xobj) = res.get_xobject(name) {
                    ...
                    } else if subtype.as_ref() == b"Image" {
    ...
    fn handle_image_xobject(&mut self, xobj: &RawXObject) -> PdfResult<()> {
        ...
        let components = match cs {
            Some(Object::Name(n)) if n.as_ref() == b"DeviceRGB" => 3,
            Some(Object::Name(n)) if n.as_ref() == b"DeviceGray" => 1,
            _ => 3,
        };
    ...
    fn handle_s_stroke(&mut self, _op: &Operation) -> PdfResult<()> {
        ...
        let (path, color_out, width, line_cap, line_join, miter_limit, dash_pattern, blend_mode, alpha) = {
            let gs = self.gs_stack.current()?;
            let intent = crate::colorspace::ColorSpace::map_intent(&gs.rendering_intent);
            let resolved_color = gs.stroke_color.to_rgb(&gs.stroke_color_space, gs.black_point_compensation, intent);
            (
    ...
    fn handle_w_star_clip(&mut self, _op: &Operation) -> PdfResult<()> {
        ...
        // Finalize the path for clipping (ISO 32000-2:2020 Clause 8.5.4)
        Arc::make_mut(&mut gs.clipping_path).extend(path.iter());
        gs.pending_clipping_rule = Some(ClippingRule::EvenOdd);
        Ok(())
    }
        ...
        if let Some(Object::Name(name)) = op.operands.first() {
            if let Some(ref res) = self.resources {
                if let Some(dict) = res.get_sub_dict(b"ExtGState") {
                    if let Some(gs_obj) = dict.get(name.as_ref()) {
    ...
    fn handle_bdc(&mut self, op: &Operation) -> PdfResult<()> {
        let tag_bytes = op.operands.first().and_then(|o| o.as_str()).unwrap_or(b"");
        let tag = String::from_utf8_lossy(tag_bytes);
        let mut visible = true;
        
        if let Some(ctx) = &self.oc_context {
             if tag == "OC" {
                 if let Some(properties) = op.operands.get(1) {
                     // Check visibility for Optional Content
                     visible = ctx.is_visible(properties, self.resolver);
                 }
             }
        }
        
        eprintln!("[DEBUG][VISIBILITY] BDC tag={}, visible={}", tag, visible);
    ...
    fn apply_ext_gstate(&mut self, dict: &BTreeMap<Vec<u8>, Object>) -> PdfResult<()> {
        ...
        let bm_raw = dict.get(b"BM".as_ref()).and_then(|o| if let Object::Name(n) = o { Some(n.as_ref()) } else { None });
