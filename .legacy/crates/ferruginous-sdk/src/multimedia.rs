...
impl<'a> Annotation3D<'a> {
    ...
    pub fn stream_3d(&self) -> Option<(std::sync::Arc<BTreeMap<Vec<u8>, Object>>, bytes::Bytes)> {
        self.dictionary.get(b"3DD".as_ref())
            .and_then(|obj| self.resolver.resolve_if_ref(obj).ok())
            .and_then(|obj| if let Object::Stream(dict, data) = obj { Some((std::sync::Arc::clone(&dict), data.clone())) } else { None })
    }
