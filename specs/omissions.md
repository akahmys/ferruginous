...
- **DCTDecode**: JPEG 形式の画像デコードをサポート（M24 で実装済）。
...
- **未実装項目**: `RunLengthDecode`, `JPXDecode`。
  - **理由**: これらは利用頻度が比較的低く、JBIG2 等の優先度の高いフィルタ実装を優先しました。
...
- **Arlington 述語 (Predicates)**: Arlington PDF Model 内に含まれる JavaScript 形式の複雑な動的条件式（述語）は、実行エンジンを内包することによる安全性の低下を防ぐため、現在は静的タイプチェックと基本的な論理性チェックに限定しています。
