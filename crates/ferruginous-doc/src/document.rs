use crate::security::{NullSecurityHandler, SecurityHandler, StandardSecurityHandler};
use crate::signature::Signature;
use crate::validation::{SignatureVerifier, ValidationStatus};
use crate::xref::{XRefEntry, XRefStore, is_pdf_whitespace, parse_xref_stream, parse_xref_table};
use bytes::Bytes;
use ferruginous_core::lexer::Token;
use ferruginous_core::{Object, Parser, PdfError, PdfResult, Reference, Resolver};
...
impl Document {
    ...
    pub fn open_with_password(data: Bytes, password: &[u8]) -> PdfResult<Self> {
                ...
                match parser.next()? {
                    Some(Token::Keyword(ref b)) if b.as_ref() == b"trailer" => {
                        let trailer = parser
                            .parse_object()?
                            .as_dict()
                            .ok_or_else(|| PdfError::Other("Invalid trailer".into()))?
                            .clone();
        ...
        let security: Arc<dyn SecurityHandler> =
            if let Some(e_obj) = store.trailer.get(&"Encrypt".into()) {
                let e_dict = match e_obj {
                    Object::Dictionary(d) => d.clone(),
                    Object::Reference(r) => {
                        let entry = store
                            .get(r.id)
                            .ok_or_else(|| PdfError::Other("Encrypt object not found".into()))?;
                        let offset = match entry {
                            XRefEntry::InUse { offset, .. } => offset,
                            _ => {
                                return Err(PdfError::Other(
                                    "Encrypt object must not be compressed or free".into(),
                                ));
                            }
                        };
                        ...
                        let d = obj.as_dict().ok_or_else(|| {
                            PdfError::Other("Encrypt object is not a dictionary".into())
                        })?;
        ...
        Ok(Self { data, store, root, object_cache: BTreeMap::new(), security, version })
    }
    ...
    pub fn open_repair(data: Bytes) -> PdfResult<Self> {
            ...
            if data[pos..].starts_with(b"obj") {
                ...
                if obj_id > 0 && !visited_ids.contains(&obj_id) {
                    store.entries.insert(
                        obj_id,
                        XRefEntry::InUse { offset: search_pos as u64, generation: generation_num },
                    );
        ...
        for id in &visited_ids {
            ...
            let entry = store
                .get(*id)
                .ok_or_else(|| PdfError::Other("Inconsistent XRef index during repair".into()))?;
            let offset = entry
                .offset()
                .ok_or_else(|| PdfError::Other("Missing offset for repair candidate".into()))?;
            let mut parser = Parser::new(data.slice(offset as usize..));
            if parser.parse_indirect_object_header().is_ok()
                && let Ok(Object::Dictionary(dict)) = parser.parse_object()
                && dict.get(&"Type".into()).and_then(|o| o.as_name()).map(|n| n.as_ref())
                    == Some(b"Catalog")
            {
                root_ref = Some(r);
                break;
        ...
        Ok(Self {
            ...
            store,
            root: root_ref
                .ok_or_else(|| PdfError::Other("Could not find Catalog during repair".into()))?,
    ...
    pub fn signatures(&self) -> PdfResult<Vec<Signature>> {
        ...
        while let Some(current_ref) = stack.pop() {
            let node = self
                .resolve(&current_ref)?
                .as_dict()
                .ok_or_else(|| PdfError::Other("Invalid page tree node".into()))?
                .clone();

            if node.get(&"Type".into()).and_then(|o| o.as_name()).map(|n| n.as_ref())
                == Some(b"Pages")
            {
                    ...
                    for annot_ref in annots.iter() {
                        if let Some(r) = annot_ref.as_reference() {
                            let annot = self
                                .resolve(&r)?
                                .as_dict()
                                .ok_or_else(|| PdfError::Other("Invalid annotation".into()))?
                                .clone();
                                ...
                                .and_then(|o| o.as_reference())
                            {
                                let sig_dict = self
                                    ...
                                    .as_dict()
                                    .ok_or_else(|| {
                                        PdfError::Other("Invalid signature dictionary".into())
                                    })?
                                    .clone();
    ...
    pub fn add_object(&mut self, obj: Object) -> Reference {
        ...
        self.store
            .entries
            .insert(next_id, crate::xref::XRefEntry::InUse { offset: 0, generation: 0 });
    ...
    fn check_mdp_compliance(&self, sig: &Signature) -> PdfResult<MdpStatus> {
        ...
        if covered_end < self.data.len() {
            ...
            if let Some(doc_mdp) = &sig.doc_mdp {
                match doc_mdp.p {
                    1 => {
                        return Ok(MdpStatus::DisallowedModifications(
                            "DocMDP Level 1: No changes permitted".into(),
                        ));
                    }
                    2 => {
                    ...
                    _ => {
                        return Ok(MdpStatus::DisallowedModifications(format!(
                            "Unknown DocMDP Level: {}",
                            doc_mdp.p
                        )));
                    }
                }
            ...
            return Ok(MdpStatus::DisallowedModifications(
                "Incremental update detected without MDP permission".into(),
            ));
        }
    ...
    pub fn explore_dependencies(
        &self,
        root: &Reference,
    ) -> PdfResult<std::collections::BTreeSet<u32>> {
    ...
    pub fn dump_structure(&self) -> PdfResult<String> {
        ...
        if let Some(Object::Reference(info_ref)) = self.store.trailer.get(&"Info".into()) {
            ...
            output.push_str(&format!(
                ...
                info_ref.id,
                info_ref.generation,
                obj.as_dict().map(|d| d.len()).unwrap_or(0)
            ));
        }
    ...
    fn resolve_compressed_object(&self, container_id: u32, index: u32) -> PdfResult<Object> {
        ...
        let (dict, _) = container_obj
            .as_stream()
            .ok_or_else(|| PdfError::Other("Object stream container is not a stream".into()))?;
        ...
        let n = dict
            .get(&"N".into())
            .and_then(|o| o.as_i64())
            .ok_or_else(|| PdfError::Other("Missing /N in ObjStm".into()))?
            as usize;
        let first = dict
            .get(&"First".into())
            .and_then(|o| o.as_i64())
            .ok_or_else(|| PdfError::Other("Missing /First in ObjStm".into()))?
            as usize;
        ...
        for i in 0..n {
            let _obj_id = parser
                .parse_object()?
                .as_i64()
                .ok_or_else(|| PdfError::Other("Invalid obj_id in ObjStm header".into()))?
                as u32;
            let offset = parser
                .parse_object()?
                .as_i64()
                .ok_or_else(|| PdfError::Other("Invalid offset in ObjStm header".into()))?
                as usize;
        ...
        let offset = target_offset.ok_or_else(|| {
            PdfError::Other(format!(
                "Compressed index {} out of range in ObjStm {}",
                index, container_id
            ))
        })?;

        // Parse the object at the calculated offset
        let mut obj_parser = Parser::new(decoded_data.slice(first + offset..)).with_resolver(self);
    ...
    fn resolve(&self, reference: &Reference) -> PdfResult<Object> {
        ...
        let mut obj = match entry {
            XRefEntry::InUse { offset, .. } => {
                let mut parser =
                    Parser::new(self.data.slice(offset as usize..)).with_resolver(self);
                parser.parse_indirect_object_header()?;
                parser.parse_object()?
            }
        ...
        if !self.is_encryption_dict(reference) {
            let skip_decryption = if let Object::Stream(dict, _) = &obj {
                let is_metadata =
                    dict.get(&"Type".into()).and_then(|o| o.as_name()).map(|n| n.as_ref())
                        == Some(b"Metadata")
                        && !self.security.encrypt_metadata();
                let is_obj_stm =
                    dict.get(&"Type".into()).and_then(|o| o.as_name()).map(|n| n.as_ref())
                        == Some(b"ObjStm");
                let is_xref =
            ...
            if !skip_decryption {
                match &mut obj {
                    Object::String(b) => {
                        let decrypted =
                            self.security.decrypt_bytes(b, reference.id, reference.generation)?;
                        *b = decrypted;
                    }
                    Object::Stream(_d, b) => {
                        let decrypted =
...
fn parse_malformed_legacy(
    ...
    match parser.next()? {
        Some(Token::Keyword(ref b)) if b.as_ref() == b"trailer" => {
            ...
            let prev =
                trailer.get(&"Prev".into()).and_then(|o: &Object| o.as_i64()).map(|i| i as usize);
            Ok((idx, prev))
        }
        _ => Ok((idx, None)),
    }
...
fn find_startxref(data: &[u8]) -> PdfResult<usize> {
    ...
    let pos = end
        ...
        .ok_or_else(|| PdfError::Syntactic { pos: 0, message: "Missing startxref".into() })?;
    let start = end_search + pos + 9;
    let s = std::str::from_utf8(&data[start..])
        .map_err(|_| PdfError::Other("Invalid UTF-8 in startxref".into()))?;
    let offset = s
        .split_whitespace()
        .next()
        .ok_or_else(|| PdfError::Syntactic { pos: start, message: "Missing offset".into() })?
        .parse::<usize>()
        .map_err(|_| PdfError::Other("Invalid offset".into()))?;
    Ok(offset)
}
