...
#[command(
    about = "High-performance ISO 32000-2 compliant PDF toolkit",
    after_help = "ISO 32000-2:2020 Compliance Enforcement Engine"
)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
    ...
    match cli.command {
            ...
            if let Ok(compliance) = doc.get_compliance() {
                ...
                if let Some(p) = compliance.metadata.pdf_a_part {
                    println!(
                        ...
                        "Standard:".bold(),
                        p,
                        compliance.metadata.pdf_a_conformance.unwrap_or_else(|| "unknown".into())
                    );
                }
                ...
                println!(
                    "{:<20} {}",
                    "Tagged PDF:".bold(),
                    if compliance.has_struct_tree { "Yes".green() } else { "No".red() }
                );
            }
        }
        Commands::Render { file, page, output } => {
            ...
            println!(
                "{} Image saved to {}",
                "Success!".green().bold(),
                out_path.display().to_string().italic()
            );
        }
        Commands::Audit { file } => {
        ...
        Commands::Extract { target } => match target {
            ExtractTarget::Text { file, page } => {
                ...
                if let Some(p_idx) = page {
                    let text =
                        doc.extract_text(p_idx).context("Failed to extract text from page")?;
                    println!("{text}");
                } else {
                    ...
                    for i in 0..count {
                        ...
                        let text = doc
                            .extract_text(i)
                            .context(format!("Failed to extract text from page {i}"))?;
                        println!("{text}");
                    }
                }
            }
