...
impl<'a, W: Write> PdfWriter<'a, W> {
    ...
    /// Sets the compression level for streams (0-9).
    ...
    pub fn write_object(&mut self, obj: &Object) -> PdfResult<()> {
        match obj {
            ...
            Object::Stream(dh, data) => {
                ...
                // Filter out keys that are inconsistent with the decompressed stream data
                let filter_key = self.arena.get_name_by_str("Filter");
                let decode_parms_key = self.arena.get_name_by_str("DecodeParms");
                ...
                self.write_all(b"<<")?;
                for (k, v) in d {
                    if Some(k) == filter_key || Some(k) == decode_parms_key || Some(k) == length_key {
                        continue;
                    }
                    self.write_all(b"\r\n")?;
                    self.write_name(&k)?;
                ...
                // Ensure /Length is correct for the data (compressed or not)
