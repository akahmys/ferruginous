...
impl OCContext {
            ...
            Object::Reference(r) => {
                ...
                match obj {
                    Object::Dictionary(ref dict) => {
                        let type_name = dict.get(b"Type".as_ref()).and_then(|o| if let Object::Name(n) = o { Some(n.as_slice()) } else { None });
                        match type_name {
    ...
    fn evaluate_ocmd(&self, dict: &BTreeMap<Vec<u8>, Object>) -> bool {
        ...
        let policy_name = dict.get(b"P".as_ref()).and_then(|o| if let Object::Name(n) = o { Some(n.as_slice()) } else { None }).unwrap_or(b"AnyOn");
