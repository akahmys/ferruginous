...
use ferruginous_core::{Handle, Matrix, Object, PdfError, PdfResult};
...
impl Interpreter<'_> {
    ...
    pub(crate) fn handle_text_scope_operator(&mut self, op: &str) -> PdfResult<()> {
        match op {
            ...
            "ET" => {
                if let Some(curr) = self.current_text_bbox {
                    self.page_text_bbox =
                        Some(self.page_text_bbox.map_or(curr, |p| p.union(&curr)));
                }
        ...
        match op {
            "Td" => {
                ...
                let m = self
                    .text_matrices
                    .as_mut()
                    .ok_or_else(|| PdfError::Other("Not in BT".into()))?;
    ...
    pub(crate) fn map_text_to_glyphs(
        ...
        text: &[u8],
        font: &FontResource,
    ) -> PdfResult<(Vec<(u32, f32, f32, f32, u32)>, String)> {
        ...
        while i < text.len() {
            ...
            if consumed == 0 || i + consumed > text.len() {
                break;
            }
