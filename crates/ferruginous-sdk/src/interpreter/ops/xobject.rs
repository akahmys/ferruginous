...
impl Interpreter<'_> {
    pub(crate) fn handle_xobject_operator(&mut self) -> PdfResult<()> {
        ...
        let entry = self.find_resource(&self.doc.arena().intern_name(PdfName::new("XObject")), &name)?;
        ...
        if let Object::Stream(dh, data) = xobj
            && let Some(dict) = self.doc.arena().get_dict(dh) {
                ...
                if let Some(sub) = dict.get(&subtype_key).and_then(|o| o.resolve(self.doc.arena()).as_name()) {
                    let sub_name = self.doc.arena().get_name(sub).ok_or_else(|| PdfError::Other("Subtype name not found".into()))?;
    ...
    pub(crate) fn render_form_xobject(&mut self, dict: &BTreeMap<Handle<PdfName>, Object>, data: &[u8]) -> PdfResult<()> {
        // 1. Save state
    ...
    pub(crate) fn render_image_xobject(&mut self, dict: &BTreeMap<Handle<PdfName>, Object>, data: &[u8]) -> PdfResult<()> {
        let width_key = self.doc.arena().intern_name(PdfName::new("Width"));
        let height_key = self.doc.arena().intern_name(PdfName::new("Height"));
        let w = u32::try_from(dict.get(&width_key).and_then(|o| o.resolve(self.doc.arena()).as_integer()).unwrap_or(0)).unwrap_or(0);
        let h = u32::try_from(dict.get(&height_key).and_then(|o| o.resolve(self.doc.arena()).as_integer()).unwrap_or(0)).unwrap_or(0);
