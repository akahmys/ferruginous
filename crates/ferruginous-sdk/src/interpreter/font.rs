...
use ferruginous_core::{PdfResult, PdfError, PdfName, Object, Handle};
...
impl Interpreter<'_> {
    pub(crate) fn resolve_font_resource(&mut self, name: &PdfName) -> PdfResult<Arc<FontResource>> {
        let entry = self.find_resource(&self.doc.arena().intern_name(PdfName::new("Font")), name)?;
        let h = entry.as_reference().unwrap_or_else(|| self.doc.arena().alloc_object(entry.clone()));
        self.get_font(h)
    }
    ...
    pub(crate) fn get_font(&mut self, h: Handle<Object>) -> PdfResult<Arc<FontResource>> {
        ...
        let mut res = if let Object::Dictionary(dfh) = font_obj {
            let dict = arena.get_dict(dfh).ok_or_else(|| PdfError::Other("Invalid font dictionary".into()))?;
        ...
        let is_cjk = res.subtype.as_str() == "Type0" || res.encoding.as_ref().is_some_and(|e| e.is_multibyte());
        let is_western = ["Century", "Arial", "Times", "Helvetica", "Courier", "Symbol", "ZapfDingbats"]
            .iter().any(|&name| res.base_font.as_str().contains(name));
        
        if !res.has_any_mapping() && (is_named || is_cjk) && !is_western
            && let Some(parent_cmap) = self.find_rescue_cmap(&res).or_else(|| self.global_rescue_cmap.clone()) {
        ...
        if !self.defined_fonts.contains(name.as_str()) {
            self.backend.define_font(name.as_str(), Some(res.base_font.as_str()), res.data.clone(), None, res.cid_to_gid_map.clone());
    ...
    pub(crate) fn scan_for_global_rescue_cmap(&mut self, res_h: Handle<BTreeMap<Handle<PdfName>, Object>>) {
        let arena = self.doc.arena();
    ...
    pub(crate) fn find_rescue_cmap(&self, current_res: &FontResource) -> Option<ferruginous_core::font::cmap::CMap> {
                        ...
                        if let Object::Dictionary(h) = obj.resolve(arena)
                            && let Some(d) = arena.get_dict(h) {
                                ...
                                if let Some(cfd) = candidate_fd
                                    ...
                                    && let Ok(data) = self.doc.decode_stream(&tu_obj.resolve(arena))
                                    && let Ok(m) = ferruginous_core::font::cmap::CMap::parse(&data) {
                                        let current_len = current_res.to_unicode.as_ref().map_or(0, |m| m.mappings.len());
