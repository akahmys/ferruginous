...
impl VelloRenderer {
    ...
    pub fn render_page(
        ...
        let tex = self.page_textures.get(&page_index).unwrap();
    ...
    fn recreate_texture(&mut self, render_state: &RenderState, page_index: usize, width: u32, height: u32) {
        let device = &render_state.device;
        ...
        self.page_textures.insert(page_index, PageTexture {
            texture,
            view,
            egui_texture: tid,
            width,
    ...
    pub fn get_texture(&self, page_index: usize) -> Option<egui::TextureId> {
        self.page_textures.get(&page_index).map(|tex| tex.egui_texture)
    }

    pub fn free_texture(&mut self, render_state: &RenderState, page_index: usize) {
        if let Some(tex) = self.page_textures.remove(&page_index) {
