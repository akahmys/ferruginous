...
impl SecurityHandler {
    ...
    pub fn new_v4(user_password: &str, o_string: &[u8], u_string: &[u8], p_value: i32, file_id: &[u8], encrypt_metadata: bool) -> PdfResult<Self> {
        let key = Self::derive_v4_key(user_password, o_string, u_string, p_value, file_id, encrypt_metadata)?;
    ...
    fn derive_v4_key(user_password: &str, o_string: &[u8], _u_string: &[u8], p_value: i32, file_id: &[u8], encrypt_metadata: bool) -> PdfResult<Vec<u8>> {
        let mut pad = [0u8; 32];
    ...
    pub fn decrypt_bytes_salted_no_salt(&self, data: &[u8], object_id: u32, generation: u16) -> PdfResult<Vec<u8>> {
    ...
    pub fn decrypt_bytes_with_salting(&self, data: &[u8], object_id: u32, generation: u16) -> PdfResult<Vec<u8>> {
        let mut key = self.encryption_key.clone();
        key.extend_from_slice(&object_id.to_le_bytes()[..3]);
        key.extend_from_slice(&generation.to_le_bytes()[..2]);
        if self.is_aes {
    ...
    pub fn decrypt_bytes_no_metadata(&self, data: &[u8], _object_id: u32, _generation: u16) -> PdfResult<Vec<u8>> {
        if let Some(ref inputs) = self.v4_inputs
             && let Ok(key) = Self::derive_v4_key(&inputs.password, &inputs.o, &inputs.u, inputs.p, &inputs.file_id, false) {
    ...
    fn decrypt_with_key(&self, data: &[u8], key: &[u8]) -> PdfResult<Vec<u8>> {
        if data.len() < 16 { return Ok(data.to_vec()); }
        let iv = &data[..16];
        let ciphertext = &data[16..];
        if ciphertext.is_empty() || !ciphertext.len().is_multiple_of(16) { return Ok(data.to_vec()); }
        ...
        let (cipher128, cipher256) = if key.len() == 16 {
            (Some(Aes128::new_from_slice(key).map_err(|_| PdfError::Other("AES-128 init fail".into()))?), None)
        } else {
            (None, Some(Aes256::new_from_slice(key).map_err(|_| PdfError::Other("AES-256 init fail".into()))?))
        };
        ...
        for chunk in ciphertext.chunks(16) {
            ...
            if let Some(c) = &cipher128 { c.decrypt_block(block_ref); }
            else if let Some(c) = &cipher256 { c.decrypt_block(block_ref); }
