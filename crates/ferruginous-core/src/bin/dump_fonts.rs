...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..doc.arena().object_count() {
        let h = ferruginous_core::Handle::new(i);
        if let Ok(Object::Dictionary(dh)) = doc.resolve(&h)
            && let Some(dict) = arena.get_dict(dh)
            && let Some(subtype) = dict.get(&arena.name("Subtype"))
            && let Some(name) = subtype.resolve(arena).as_name()
        {
            ...
            if name_str == "Type1" || name_str == "TrueType" || name_str == "Type0" {
                ...
                if let Some(enc_obj) = dict.get(&arena.name("Encoding")) {
                    ...
                    if let Object::Dictionary(eh) = resolved_enc
                        && let Some(edict) = arena.get_dict(eh)
                        && let Some(diffs_obj) = edict.get(&arena.name("Differences"))
                    {
                            ...
                            && let Some(arr) = arena.get_array(ah)
                        {
                            ...
                            for item in arr {
                                match item.resolve(arena) {
                                    ...
                                    Object::Name(nh) => {
                                        print!("/{} ", arena.get_name(nh).unwrap().as_str())
                                    }
                                    _ => {}
                                }
                            }
                ...
                if let Some(fd_obj) = dict.get(&arena.name("FontDescriptor"))
                    && let Object::Dictionary(fdh) = fd_obj.resolve(arena)
                {
                    ...
                    for key in ["FontFile", "FontFile2", "FontFile3"] {
                        if let Some(ff) = fdict.get(&arena.name(key)) {
                            println!("    {}: Present", key);
                            if key == "FontFile3"
                                && let Object::Stream(sh, _) = ff.resolve(arena)
                            {
