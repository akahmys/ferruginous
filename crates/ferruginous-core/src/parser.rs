...
impl<'a> Parser<'a> {
    ...
    fn parse_object_internal(&mut self) -> PdfResult<Object> {
        let t = self.next()?.ok_or_else(|| PdfError::Syntactic { pos: 0, message: "Unexpected EOF".into() })?;
    ...
    pub fn parse_indirect_object_header(&mut self) -> PdfResult<(u32, u16)> {
        let id_token = self.next()?.ok_or_else(|| PdfError::Syntactic { pos: 0, message: "Expected object ID".into() })?;
        let gen_token = self.next()?.ok_or_else(|| PdfError::Syntactic { pos: 0, message: "Expected generation number".into() })?;
        
        let id = match id_token {
            Token::Integer(i) => i as u32,
            _ => return Err(PdfError::Syntactic { pos: 0, message: format!("Expected integer object ID, found {:?}", id_token) }),
        };
        
        let generation = match gen_token {
            Token::Integer(i) => i as u16,
            _ => return Err(PdfError::Syntactic { pos: 0, message: format!("Expected integer generation number, found {:?}", gen_token) }),
        };

        match self.next()? {
            Some(Token::Keyword(b)) if b.as_ref() == b"obj" => Ok((id, generation)),
            Some(t) => Err(PdfError::Syntactic { pos: 0, message: format!("Expected 'obj' keyword, found {:?}", t) }),
    ...
    fn parse_array(&mut self) -> PdfResult<Object> {
        ...
        loop {
            match self.peek()? {
                ...
                None => return Err(PdfError::Syntactic { pos: 0, message: "Unterminated array".into() }),
                _ => {
    ...
    fn parse_dictionary_or_stream(&mut self) -> PdfResult<Object> {
        ...
        loop {
            match self.peek()? {
                ...
                None => return Err(PdfError::Syntactic { pos: 0, message: "Unterminated dictionary".into() }),
                Some(Token::Solidus) => {
                    self.next()?; // consume '/'
                }
                Some(Token::Name(_)) => {
                ...
                _ => {
                    let t = self.next()?;
                    return Err(PdfError::Syntactic { pos: 0, message: format!("Expected key (Name) or >>, found {:?}", t) });
                }
            }
        }
    ...
    fn lex_stream_data(&mut self, dict: Arc<BTreeMap<PdfName, Object>>) -> PdfResult<Object> {
        ...
        // Resolve Length
        let length_obj = dict.get(&"Length".into()).ok_or_else(|| PdfError::Other("Missing /Length in stream".into()))?;
        let length = match length_obj {
            Object::Integer(i) => *i as usize,
            Object::Reference(r) => {
                if let Some(resolver) = self.resolver {
                    let resolved = resolver.resolve(r)?;
                    resolved.as_i64().ok_or_else(|| PdfError::Other("Invalid /Length resolved".into()))? as usize
                } else {
        ...
        if length > stream_input.len() {
            return Err(PdfError::Other(format!("Stream data too short (expected {}, got {})", length, stream_input.len())));
        }
        ...
        match self.next()? {
            ...
            t => return Err(PdfError::Syntactic { pos: 0, message: format!("Expected endstream, found {:?}", t) }),
        }
