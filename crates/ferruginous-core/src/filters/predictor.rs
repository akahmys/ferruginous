...
pub fn apply_predictor(data: &[u8], params: &Object, arena: &PdfArena) -> PdfResult<Vec<u8>> {
    let dict = params.as_dict_handle()
...
fn get_int_param(dict: &BTreeMap<Handle<PdfName>, Object>, arena: &PdfArena, key: &str, default: i64) -> i64 {
    arena.get_name_by_str(key)
        .and_then(|handle| dict.get(&handle))
        .map(|obj| obj.resolve(arena).as_integer().unwrap_or(default))
        .unwrap_or(default)
}

fn decode_png_predictor(data: &[u8], dict: &BTreeMap<Handle<PdfName>, Object>, arena: &PdfArena) -> PdfResult<Vec<u8>> {
