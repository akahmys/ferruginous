            ...
            match raw_decoder.by_ref().take(MAX_DECODE_SIZE as u64).read_to_end(&mut raw_decoded) {
                Ok(_) => {
                    if raw_decoded.len() == MAX_DECODE_SIZE {
                        return Err(PdfError::Other("Maximum decode size exceeded (Raw Deflate)".into()));
                    }
                    Ok(raw_decoded)
                }
                Err(raw_e) => {
                    Err(PdfError::Other(format!("FlateDecode failed: Zlib error ({}), Raw Deflate error ({})", e, raw_e)))
                }
            }
        }
    }
