...
pub fn decode_stream_from_dict(
    ...
    let filters = match dict.get(&crate::PdfName::new(b"Filter")) {
        ...
        Some(crate::Object::Array(arr)) => arr.iter().filter_map(|o| o.as_name().map(|n| n.as_str().to_string())).collect(),
        _ => vec![],
    };

    let parms = match dict.get(&crate::PdfName::new(b"DecodeParms")).or_else(|| dict.get(&crate::PdfName::new(b"DP"))) {
    ...
    for (i, filter) in filters.iter().enumerate() {
        ...
        if let Some(parm_dict) = parms.get(i).and_then(|p| p.as_ref()) {
            ...
            if predictor > 1 {
                ...
                data = crate::filters::predict::decode_predictor(predictor, colors, bits_per_component, columns, &data)?;
            }
        }
    }
