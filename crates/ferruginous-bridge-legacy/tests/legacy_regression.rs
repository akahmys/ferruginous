...
fn test_sjis_normalization() {
    use ferruginous_bridge_legacy::lopdf::{Object, Document, object::StringFormat};
    use bytes::Bytes;
    ...
    if let Some(Object::String(normalized, _)) = doc.get_object(1) {
        let s = std::str::from_utf8(normalized).expect("Normalization should result in valid UTF-8");
        assert_eq!(s, "日本語");
    } else {
...
fn test_corruption_recovery() {
    ...
    let doc_no_header = Reader::load_document(&data_no_header).expect("Should recover no_header.pdf");
    ...
    let doc_broken_xref = Reader::load_document(&data_broken_xref).expect("Should recover broken_xref_offset.pdf");
